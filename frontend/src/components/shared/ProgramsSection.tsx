...
export const ProgramsSection: React.FC<ProgramsSectionProps> = ({
  ...
  const getActiveColor = () => {
    if (location.pathname.includes('football')) return 'border-green-500 bg-green-500/10';
    if (location.pathname.includes('basketball')) return 'border-orange-500 bg-orange-500/10';
    if (location.pathname.includes('events')) return 'border-purple-500 bg-purple-500/10';
    if (location.pathname.includes('mun')) return 'border-blue-500 bg-blue-500/10';
    if (location.pathname.includes('racing')) return 'border-amber-500 bg-amber-500/10';
            ...
            >
              <div className={`relative bg-gray-900/80 backdrop-blur-md rounded-3xl p-10 h-full shadow-2xl hover:shadow-[0_20px_60px_${accentColor.includes('amber') ? 'rgba(245,158,11,0.4)' : 'rgba(34,197,94,0.4)'}] transition-all duration-500 border-4 ${accentColor.includes('amber') ? 'border-amber-500/20 hover:border-amber-500/50' : 'border-green-500/20 hover:border-green-500/50'} overflow-hidden`}>
                ...
                <div className="space-y-6 mb-8">
                  <h3 className="text-4xl font-black text-white uppercase tracking-tight pr-20" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>
                    {program.title}
                  </h3>
                  
                  <div className={`inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r ${program.color} text-white rounded-full text-sm font-black uppercase`}>
                ...
                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  <Button className={`bg-gradient-to-r ${program.color} hover:shadow-xl text-white font-black uppercase w-full group/btn`}>
                    Learn More
                ...
                {/* Glow Effect */}
                <div className={`absolute inset-0 rounded-3xl bg-gradient-to-br ${program.color} opacity-0 group-hover:opacity-20 blur-2xl transition-opacity duration-500 -z-10`} />
              </div>
            </motion.div>
          ))}
        </div>
